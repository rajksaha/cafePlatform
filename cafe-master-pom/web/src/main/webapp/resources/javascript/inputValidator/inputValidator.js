validator = {
	...
	markError : function(control, isValid, errorType) {
		if (isValid) {
			...
			$(control).parents(".form-group").find('.validationErrMsg').addClass("hidden");
			return 0;
		} else {
			$(control).parents(".form-group").find('.validationErrMsg').removeClass("hidden");
			var msg = validator.defaultErrors[errorType];
			$(control).parents(".form-group").find('.validationErrMsg').html(msg);
